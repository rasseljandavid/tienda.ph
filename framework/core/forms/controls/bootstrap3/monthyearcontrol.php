...
class monthyearcontrol extends formcontrol {
	...
	function toHTML($label,$name) {
        ...
        $html .= "<label class=\"".(bs3()?"control-label":"") . (($this->horizontal == 1)?' col-sm-2':'') ."\"><span class=\"".(bs3()?"control-label":"label")."\">" . $label . "</span></label>";
	...
	function controlToHTML($name,$label=null) {
		$html = '<select ' . (bs3() ? 'class="form-control" ' : '') . (($this->horizontal == 1)?'style="display:inline;" ':'') . 'id="' . $name . '_month" name="' . $name . '_month">';
		for ($i = 1; $i <= 12; $i++) {
			$s = ((strlen($i) == 1)?"0".$i:$i);
			$html .= '<option value="' . $s . '"';
		...
		$html .= '</select>';
		$html .= "/";
		$html .= '<select ' . (bs3() ? 'class="form-control" ' : '') . (($this->horizontal == 1)?'style="display:inline;" ':'') . 'id="' . $name . '_year" name="' . $name . '_year">';
