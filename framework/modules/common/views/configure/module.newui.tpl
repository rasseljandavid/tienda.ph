...
{if !$hcview}
    {control type=dropdown id="actions" name=actions items=$actions size=count($actions) value=$container->action label="Content Action"|gettext}
    {control type=dropdown id="views" name=views items=$mod_views size=count($mod_views) value=$container->view label="Content Display"|gettext}
