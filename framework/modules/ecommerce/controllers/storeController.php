...
//			
        ...
        $sql .= 'sc.storecategories_id IN (';
        //$sql .= 'SELECT id FROM ' . DB_TABLE_PREFIX . '_storeCategories WHERE rgt BETWEEN ' . $this->category->lft . ' AND ' . $this->category->rgt . ')';
		$sql .= 'SELECT id FROM ' . DB_TABLE_PREFIX . "_storeCategories WHERE id = {$this->params['category']})";
		$sql .= "ORDER BY rank ASC LIMIT $startingplace, $limit";
        $sql = $sql_start . $sql;
		
		$recs = $db->selectObjectsBySql($sql);
