...
//   }
   ...
   function query_time ($func = '', $displ = FALSE) {
        $qt = 0;
        $qn = 0;
        $res = @mysqli_query($this->mysqldb->connection,"SHOW profiles");
   ...
   function __construct($username, $password, $hostname, $database, $new=false, $log_file=null) {
		//$log_file==null ? $this->$logFile = BASE . '/tmp/sql.log' : $log_file;
	    ...
	    $this->mysqldb = new mysqli_database($username, $password, $hostname, $database, $new=false);
//	    list ( $host, $port ) = @explode (":", $hostname);
//		if ($this->connection = mysqli_connect($host, $username, $password, $database, $port)) {
//			$this->havedb = true;
//		}
	    if ($this->mysqldb->connection) {
			$this->havedb = true;
	 	}
...
//		}
        ...
        @mysqli_query($this->mysqldb->connection,"SET profiling = 1");
	}

	public function __call($name, $arguments)
	{
		$return = call_user_func_array(array($this->mysqldb, $name), $arguments);
		$this->query_time($name);
		return $return;
	}
